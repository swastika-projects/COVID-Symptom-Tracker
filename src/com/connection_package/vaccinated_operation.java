...
public class vaccinated_operation {
	...
	public static void delete(vaccinated v1)
	{
		try{
			...
			//reset foreign key check to 1
			smt1.executeUpdate("set FOREIGN_KEY_CHECKS=1");
		}catch(Exception e){
			e.printStackTrace();
			}
	}
	...
	public static void VaccinatedGetInfo(int vid)
	{
		try{
			Connection con = connection.create_connection();
			PreparedStatement pstmt = con.prepareStatement("select * from days where vid=?");
		    pstmt.setInt(1, vid);
			ResultSet set = pstmt.executeQuery();
			...
			str += "day15 = " + days[15] + " and vid is not null";
